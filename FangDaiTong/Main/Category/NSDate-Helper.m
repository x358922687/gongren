...
- (NSString *)dateStringForYearMonthDayHourMinSec
{
    return [self stringWithFormat:@"YYYYMMddHHmmss"];
}

- (NSString *)dateStringForYearMonthDayHourMin
{
...
+ (NSDate *)timeoffMin:(int)min date:(NSDate *)date
{
    NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar];
    NSDateComponents *offsetComponents = [[NSDateComponents alloc] init];
    [offsetComponents setMinute:min];
    NSDate *endTime = [gregorian dateByAddingComponents:offsetComponents toDate:date options:0];
    return endTime;
}
//拼接最晚营业时间
+ (NSDate *)dateWithDay:(NSString *)day serviceTime:(NSString *)time maxTime:(BOOL)range
{
    NSArray *array = [time componentsSeparatedByString:@"-"];
    NSString *str = range ? [array lastObject] : [array firstObject];
    NSString *maxStr = [str stringByAppendingString:@":00"];
    NSString *max = [day stringByAppendingString:[NSString stringWithFormat:@" %@",maxStr]];
    NSDate *maxDate = [NSDate dateFromString:max withFormat:@"yyyy-MM-dd HH:mm:ss"];
    return maxDate;
}
