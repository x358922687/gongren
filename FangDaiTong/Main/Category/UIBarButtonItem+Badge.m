...
- (void)badgeInit
{
    ...
    self.badgePadding   = 6;
    self.badgeMinSize   = 8;
    CGFloat speacX = self.badgeValue.length == 3 ? 8.0f : 0.0f;
    self.badgeOriginX   = self.customView.frame.size.width - self.badge.frame.size.width/ 2 - speacX;
    self.badgeOriginY   = 2;
...
-(void) setBadgeValue:(NSString *)badgeValue
{
    ...
    if (!badgeValue || [badgeValue isEqualToString:@""] || ([badgeValue isEqualToString:@"0"] && self.shouldHideBadgeAtZero)) {
        ...
        CGFloat badgeOriginX = self.badgeOriginX;
        self.badge                      = [[UILabel alloc] initWithFrame:CGRectMake(badgeOriginX, self.badgeOriginY, 5, 5)];
        self.badge.textColor            = self.badgeTextColor;
        self.badge.backgroundColor      = self.badgeBGColor;
        self.badge.font                 = self.badgeFont;
